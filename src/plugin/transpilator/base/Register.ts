...
export class RegisterBuild {
    ...
    private static converClasNameToTag(file: string, classInfo: ClassInfo, project: ProjectUtils): string {
        ...
        const name = className.replace(/([a-z])([A-Z])/g, '$1-$2').replace(/(\d+)([A-Z])/g, '$1-$2').toLowerCase();
        let tag = name;
    ...
    private static async readRegister(fileInfo: FileInfo, classInfo: ClassInfo, project: ProjectUtils) {
        ...
        if (classInfo.registerOptions.tag == undefined)
            classInfo.registerOptions.tag = this.converClasNameToTag(fileInfo.path, classInfo, project);
        else
           project.putTag(classInfo.registerOptions.tag, fileInfo.path);
