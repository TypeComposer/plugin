...
export class InfoComponent {
            ...
            if (Node.isCallExpression(node)) {
                ...
                if (Node.isPropertyAccessExpression(expr)) {
                    ...
                    if (args.length >= 2 && args[1] === data.extendsName) {
                        const info = {...(args.length > 2 ? JSON.parse(args[2].replace(/([{,]\s*)([a-zA-Z0-9_]+)\s*:/g, '$1"$2":').replace(/'/g, '"')) : {}), tag: args[0].replace(/^['"]|['"]$/g, "")};
    ...
    private static getExtendsInfo(classDeclaration: ClassDeclaration, project: ProjectBuild): IExtendsInfo  {
        ...
        if (!data.options)
        {
            ...
            if (classDeclaration)
                data.options = (this.getExtendsInfo(classDeclaration, project))?.options;
        }
        return data;
    }
    ...
    private static getClassInfo(classDeclaration: ClassDeclaration,  project: ProjectBuild): ClassInfo {
        ...
        if (classInfo.registerOptions)
            delete classInfo.registerOptions.tag;
        return classInfo
    }
