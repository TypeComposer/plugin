...
export class ProjectUtils extends Project {
  ...
  public readonly enabledTemplate: boolean;
  public jsx: ts.JsxEmit;
  ...
  constructor(public options: Options) {
    ...
    const tsConfig = this.getCompilerOptions();
    this.jsx = tsConfig.jsx || ts.JsxEmit.None;
    this.enabledTemplate = tsConfig.jsx === ts.JsxEmit.Preserve || tsConfig.jsxFactory === "TypeComposer.createElement" || tsConfig.jsxFragmentFactory === "TypeComposer.Fragment";
    Debuger.log("EnabledTemplate", this.enabledTemplate, tsConfig.jsx, " jsxFactory: ", tsConfig.jsxFactory, " jsxFragmentFactory: ", tsConfig.jsxFragmentFactory);
    this.pathName =
      this.getSourceFiles()
        .find((e) => e.getFilePath().includes("node_modules/typecomposer-plugin"))
        ?.getFilePath() || "";
    if (this.pathName != "") this.pathName = this.pathName.split("node_modules/typecomposer-plugin/")[0] + "node_modules/typecomposer-plugin/";
  ...
  validTag(tag: string, file: string): boolean {
    ...
    for (const file of remove) this.dynamicTags.delete(file);
  ...
  public async load_node_modules_dependencys() {
    ...
    try {
      ...
      const packets = items
        .filter((item) => statSync(path.join("node_modules", item)).isDirectory() && item !== "." && item !== "..")
        .map((item) => path.join("node_modules", path.join(item, "package.json")))
        .filter((item) => existsSync(item));
      await Promise.all(
        packets.map((packageJsonPath) => {
          const packageJsonContent = readFileSync(packageJsonPath, "utf-8");
          const packageJson = JSON.parse(packageJsonContent);
          const dependencies = packageJson.dependencies || {};
          const devDependencies = packageJson.devDependencies || {};
          if (dependencies?.typecomposer || devDependencies?.typecomposer) listDependencies.push(packageJsonPath.replace("package.json", ""));
        })
      );
    } catch (error) {
  ...
  public async getImportInfo(sourceFile: SourceFile): Promise<ImportInfo[]> {
    ...
    for (const imp of imports) {
      if (imp.moduleSourcePath.includes("?v=")) imp.moduleSourcePath = imp.moduleSourcePath.split("?v=")[0];
    }
    return imports;
  }
